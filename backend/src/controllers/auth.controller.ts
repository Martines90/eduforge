...
function serializeSubscription(subscription: UserDocument["subscription"]) {
  ...
  return {
    ...
    startDate:
      subscription.startDate?.toDate?.()?.toISOString() ||
      subscription.startDate,
    endDate:
      subscription.endDate?.toDate?.()?.toISOString() || subscription.endDate,
...
export async function sendVerificationCode(
  ...
  try {
    ...
    res.status(400).json({
      success: false,
      message:
        "This endpoint is deprecated. Please use /api/auth/register to initiate registration.",
    } as AuthResponse);
  } catch (error: any) {
...
export async function verifyEmail(req: Request, res: Response): Promise<void> {
  try {
    ...
    const { uid, token } = await authService.verifyCodeAndCreateUser(
      email,
      code
    );
 ...
 */
export async function trackGuestTaskView(
  req: Request,
  res: Response
): Promise<void> {
  try {
    ...
    console.log(
      "[Guest Task View] Tracking view for fingerprint:",
    ...
    console.log("[Guest Task View] View tracked:", {
      viewCount,
      viewsRemaining,
    });
