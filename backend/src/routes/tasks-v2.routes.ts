 ...
 */
router.post('/api/v2/tasks', authenticateUser, requireTeacher, async (req: Request, res: Response) => {
  try {
    const user = (req as any).user;
 ...
 */
router.get('/api/v2/tasks/my', authenticateUser, requireTeacher, async (req: Request, res: Response) => {
  try {
    const user = (req as any).user;

    const query: GetTasksQuery = {
      createdBy: user.uid,
      isPublished: req.query.status === 'published' ? true : req.query.status === 'draft' ? false : undefined,
      sort: (req.query.sort as any) || 'recent',
      limit: req.query.limit ? parseInt(req.query.limit as string) : 20,
      offset: req.query.offset ? parseInt(req.query.offset as string) : 0,
    };
 ...
 */
router.put('/api/v2/tasks/:id', authenticateUser, requireTeacher, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const user = (req as any).user;
 ...
 */
router.delete('/api/v2/tasks/:id', authenticateUser, requireTeacher, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const user = (req as any).user;
 ...
 */
router.post('/api/v2/tasks/:id/rate', authenticateUser, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const user = (req as any).user;
