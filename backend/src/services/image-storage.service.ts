...
export class ImageStorageService {
  ...
  constructor() {
    console.log(
      `[ImageStorage] Using Firebase Storage bucket: ${this.bucket.name}`
    );
  }
  ...
  async storeImagePermanently(
    ...
    try {
      ...
      console.log(
        `☁️  [ImageStorage] Uploading to Firebase Storage: ${filename}`
      );
   ...
   */
  async storeImagesPermanently(
    temporaryUrls: string[],
    taskId: string
  ): Promise<string[]> {
    console.log(
      `📥 [ImageStorage] Storing ${temporaryUrls.length} image(s) permanently...`
    );
