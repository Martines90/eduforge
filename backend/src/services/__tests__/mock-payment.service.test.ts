...
describe("Mock Payment Service E2E Tests", () => {
  ...
  describe("handleMockCheckoutSuccess", () => {
    ...
    it("should create Stripe mock IDs", async () => {
      ...
      const updateCall = mockFirestore.update.mock.calls[0][0];
      expect(updateCall.subscription.stripeCustomerId).toContain(
        "mock_customer_"
      );
      expect(updateCall.subscription.stripeSubscriptionId).toContain(
        "mock_sub_"
      );
      expect(updateCall.subscription.stripePriceId).toContain(
        "mock_price_normal"
      );
    });
  ...
  describe("cancelMockSubscription", () => {
    ...
    it("should reject if user not found", async () => {
      ...
      await expect(cancelMockSubscription("non-existent-user")).rejects.toThrow(
        "User not found"
      );
    });
    ...
    it("should reject if no active subscription", async () => {
      ...
      await expect(cancelMockSubscription("test-user-123")).rejects.toThrow(
  ...
  describe("reactivateMockSubscription", () => {
    ...
    it("should reject if no subscription", async () => {
      ...
      await expect(reactivateMockSubscription("test-user-123")).rejects.toThrow(
