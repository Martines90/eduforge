import { Translations } from '@/types/i18n';

export const mx: Translations = {
  // Navigation
  'Home': 'Inicio',
  'Task Creator': 'Creador de Tareas',
  'Open navigation menu': 'Abrir menú de navegación',
  'Close menu': 'Cerrar menú',

  // Home page
  'Educational Task Platform': 'Plataforma de Tareas Educativas',
  'Create educational tasks based on curriculum topics for grades 9-12': 'Crea tareas educativas basadas en temas curriculares para grados 9-12',
  'Go to Task Creator': 'Ir al Creador de Tareas',
  'Search Tasks': 'Buscar Tareas',
  'Browse and discover educational tasks created by teachers': 'Explora y descubre tareas educativas creadas por maestros',
  'Browse Tasks': 'Explorar Tareas',

  // Task Creator page
  'Select a curriculum topic to create an educational task': 'Selecciona un tema curricular para crear una tarea educativa',
  'Select Topic': 'Seleccionar Tema',
  'Grade 9-10': 'Grado 9-10',
  'Grade 11-12': 'Grado 11-12',
  'Reset': 'Restablecer',
  'Selection complete': 'Selección completa',
  'Please select a topic to begin': 'Por favor selecciona un tema para comenzar',
  'Confirm Selection': 'Confirmar Selección',
  'Topic': 'Tema',
  'Path': 'Ruta',
  'Grade Level': 'Nivel de Grado',
  'Selected Topic': 'Tema Seleccionado',
  'Create Task': 'Crear Tarea',
  'Clear Selection': 'Limpiar Selección',

  // Select component
  'Select an option': 'Selecciona una opción',
  'Select main topic': 'Selecciona el tema principal',
  'Select sub-topic': 'Selecciona el subtema',
  'Level': 'Nivel',

  // General
  'Close': 'Cerrar',
  'Open': 'Abrir',
  'Menu': 'Menú',
  'Language': 'Idioma',
  'Country': 'País',

  // Tasks page
  'Tasks': 'Tareas',
  'Educational Tasks': 'Tareas Educativas',
  'Browse and explore educational tasks for grades 9-12': 'Explora y descubre tareas educativas para grados 9-12',
  'Search tasks...': 'Buscar tareas...',
  'Subject': 'Materia',
  'Grade': 'Grado',
  'All Subjects': 'Todas las Materias',
  'All Grades': 'Todos los Grados',
  'No Tasks Available Yet': 'No Hay Tareas Disponibles Aún',
  'Educational tasks will appear here once teachers start creating them. Check back soon!': '¡Las tareas educativas aparecerán aquí una vez que los maestros comiencen a crearlas. Vuelve pronto!',
  'Back to Home': 'Volver al Inicio',
  'Start Task': 'Comenzar Tarea',
  'Difficulty': 'Dificultad',
  'Medium': 'Medio',
  'Tree View': 'Vista de Árbol',
  'Grid View': 'Vista de Cuadrícula',
  'Category / Task Title': 'Categoría / Título de Tarea',
  'School System': 'Sistema Escolar',
  'Rating': 'Calificación',
  'Views': 'Vistas',
  'Loading tasks...': 'Cargando tareas...',
  'No tasks available for this topic yet': 'No hay tareas disponibles para este tema aún',
  'No teacher added any tasks yet.': 'Ningún maestro ha agregado tareas aún.',
  'Be the first one who creates a new task': 'Sé el primero en crear una nueva tarea',

  // Subjects
  'Mathematics': 'Matemáticas',
  'Physics': 'Física',
  'Chemistry': 'Química',
  'Biology': 'Biología',
  'History': 'Historia',
  'Geography': 'Geografía',
  'Literature': 'Literatura',

  // Profile page
  'My Profile': 'Mi Perfil',
  'View and manage your account information': 'Ver y gestionar la información de tu cuenta',
  'Personal Information': 'Información Personal',
  'Name': 'Nombre',
  'Email': 'Correo Electrónico',
  'Role': 'Rol',
  'Teacher': 'Maestro',
  'Student / General User': 'Estudiante / Usuario General',
  'Password & Security': 'Contraseña y Seguridad',
  'Keep your account secure by changing your password regularly.': 'Mantén tu cuenta segura cambiando tu contraseña regularmente.',
  'Change Password': 'Cambiar Contraseña',
  'Current Password': 'Contraseña Actual',
  'New Password': 'Nueva Contraseña',
  'Confirm New Password': 'Confirmar Nueva Contraseña',
  'Must be at least 8 characters long': 'Debe tener al menos 8 caracteres',
  'Cancel': 'Cancelar',
  'Changing Password...': 'Cambiando Contraseña...',
  'Password changed successfully!': '¡Contraseña cambiada exitosamente!',
  'View My Tasks': 'Ver Mis Tareas',

  // My Tasks page
  'My Tasks': 'Mis Tareas',
  'View and manage all tasks you\'ve created': 'Ver y gestionar todas las tareas que has creado',
  'This page will display all educational tasks you have created. You\'ll be able to view, edit, and manage your tasks from here.': 'Esta página mostrará todas las tareas educativas que has creado. Podrás ver, editar y gestionar tus tareas desde aquí.',
  'No Tasks Yet': 'No Hay Tareas Aún',
  'You haven\'t created any tasks yet. Start by creating your first educational task!': '¡No has creado ninguna tarea aún. Comienza creando tu primera tarea educativa!',
  'Create Your First Task': 'Crea Tu Primera Tarea',
  'Create New Task': 'Crear Nueva Tarea',
  'View Profile': 'Ver Perfil',
  'Created': 'Creado',

  // User Menu
  'Profile': 'Perfil',
  'My Subscription': 'Mi Suscripción',
  'Logout': 'Cerrar Sesión',

  // Additional Profile page
  'Not provided': 'No proporcionado',
  'Loading...': 'Cargando...',
  'Loading profile...': 'Cargando perfil...',
  'Redirecting...': 'Redirigiendo...',
  'All password fields are required': 'Todos los campos de contraseña son obligatorios',
  'New password must be at least 8 characters long': 'La nueva contraseña debe tener al menos 8 caracteres',
  'New password must be different from old password': 'La nueva contraseña debe ser diferente de la anterior',
  'New passwords do not match': 'Las nuevas contraseñas no coinciden',
  'Failed to change password. Please check your old password and try again.': 'Error al cambiar la contraseña. Por favor verifica tu contraseña anterior e intenta de nuevo.',
  'Password change functionality will be implemented with backend API': 'La funcionalidad de cambio de contraseña se implementará con la API del backend',

  // Educational Models
  'Educational Model': 'Modelo Educativo',
  'Select educational model': 'Selecciona el modelo educativo',
  'Secular': 'Secular',
  'Conservative': 'Conservador',
  'Traditional': 'Tradicional',
  'Liberal': 'Liberal',
  'Progressive': 'Progresivo',
  'Religious - Christian': 'Religioso - Cristiano',
  'Religious - Islamic': 'Religioso - Islámico',
  'Religious - Jewish': 'Religioso - Judío',
  'Montessori': 'Montessori',
  'Waldorf': 'Waldorf',

  // Registration Modal
  'Create Your Account': 'Crea Tu Cuenta',
  'Country & Subject': 'País y Materia',
  'Location and expertise': 'Ubicación y experiencia',
  'Personal Info': 'Información Personal',
  'Name and Email': 'Nombre y Correo',
  'Verify Email': 'Verificar Correo',
  'Enter verification code': 'Ingresa el código de verificación',
  'Select your location': 'Selecciona tu ubicación',
  'Select Your Country & Subject': 'Selecciona Tu País y Materia',
  'Select Your Country': 'Selecciona Tu País',
  'Choose your location and primary teaching subject': 'Elige tu ubicación y materia principal de enseñanza',
  'Choose your country to personalize your experience': 'Elige tu país para personalizar tu experiencia',
  'Your Work Information': 'Tu Información Laboral',
  'Your Information': 'Tu Información',
  'Full Name': 'Nombre Completo',
  'Enter your full name': 'Ingresa tu nombre completo',
  'Work Email Address': 'Correo Electrónico de Trabajo',
  'Email Address': 'Correo Electrónico',
  'Password': 'Contraseña',
  'At least 6 characters': 'Al menos 6 caracteres',
  'Confirm Password': 'Confirmar Contraseña',
  'Re-enter your password': 'Vuelve a ingresar tu contraseña',
  'Back': 'Atrás',
  'Sending Code...': 'Enviando Código...',
  'Create Account': 'Crear Cuenta',
  'Verify Your Email': 'Verifica Tu Correo',
  'We\'ve sent a 6-digit verification code to': 'Hemos enviado un código de verificación de 6 dígitos a',
  'Please enter a valid 6-digit code': 'Por favor ingresa un código válido de 6 dígitos',
  'Check the console for your verification code (email sending not yet implemented)': 'Revisa la consola para tu código de verificación (el envío de correo aún no está implementado)',
  'Verifying...': 'Verificando...',
  'Verify & Create Account': 'Verificar y Crear Cuenta',
  'Back to Login': 'Volver al Inicio de Sesión',
  'Next': 'Siguiente',
  'Failed to register user. Please try again.': 'Error al registrar usuario. Por favor intenta de nuevo.',
  'Failed to verify code. Please try again.': 'Error al verificar código. Por favor intenta de nuevo.',
  'Registration successful! Welcome to EduForge.': '¡Registro exitoso! Bienvenido a EduForge.',

  // Action Selection Modal
  'What would you like to do?': '¿Qué te gustaría hacer?',
  'Choose your next step for': 'Elige tu siguiente paso para',
  'Continue': 'Continuar',

  // Target Group
  'Target Group': 'Grupo Objetivo',
  'Mixed (Boys and Girls)': 'Mixto (Niños y Niñas)',
  'Boys Only': 'Solo Niños',
  'Girls Only': 'Solo Niñas',

  // Image Number
  'Number of Images': 'Número de Imágenes',
  'No images (text only)': 'Sin imágenes (solo texto)',
  '1 image': '1 imagen',
  '2 images': '2 imágenes',

  // Task Settings
  'Task Settings': 'Configuración de Tarea',
  'Difficulty Level': 'Nivel de Dificultad',
  'Generate Task': 'Generar Tarea',

  // Task Generation Progress Messages
  'Generating task...': 'Generando tarea...',
  'Generating 3 task variations in parallel...': 'Generando 3 variaciones de tarea en paralelo...',
  '3 task variations successfully generated!': '¡3 variaciones de tarea generadas exitosamente!',
  'AI is selecting the best variation...': 'La IA está seleccionando la mejor variación...',
  'Best variation selected (variation': 'Mejor variación seleccionada (variación',
  'Generating images...': 'Generando imágenes...',
  'Generating images and solution...': 'Generando imágenes y solución...',
  'Generating solution...': 'Generando solución...',
  'Task successfully completed!': '¡Tarea completada exitosamente!',
  'Please wait...': 'Por favor espera...',

  // Task Result Component
  'Generated Task': 'Tarea Generada',
  'Edit task': 'Editar tarea',
  'Edit solution': 'Editar solución',
  'Save': 'Guardar',
  'characters': 'caracteres',
  '{{count}} more characters needed': '{{count}} caracteres más necesarios',
  '{{count}} characters over limit': '{{count}} caracteres sobre el límite',
  'Task description is too short! At least {{min}} characters required. Current: {{count}} characters.': '¡La descripción de la tarea es demasiado corta! Se requieren al menos {{min}} caracteres. Actual: {{count}} caracteres.',
  'Task description is too long! Maximum {{max}} characters allowed. Current: {{count}} characters.': '¡La descripción de la tarea es demasiado larga! Máximo {{max}} caracteres permitidos. Actual: {{count}} caracteres.',
  'An error occurred while generating the PDF. Please try again.': 'Ocurrió un error al generar el PDF. Por favor intenta de nuevo.',
  'An error occurred': 'Ocurrió un error',
  'Save Task': 'Guardar Tarea',
  'Saving...': 'Guardando...',
  'Questions': 'Preguntas',
  'Solution Steps': 'Pasos de Solución',
  'Formula': 'Fórmula',
  'Calculation': 'Cálculo',
  'Result': 'Resultado',
  'Final Answer': 'Respuesta Final',
  'Verification': 'Verificación',
  'Common Mistakes': 'Errores Comunes',

  // Task Saved Modal
  'Task Saved Successfully!': '¡Tarea Guardada Exitosamente!',
  'Your task has been saved to the database and is now available publicly.': 'Tu tarea ha sido guardada en la base de datos y ahora está disponible públicamente.',
  'Public Share Link:': 'Enlace Público para Compartir:',
  'Link copied to clipboard!': '¡Enlace copiado al portapapeles!',
  'Task ID': 'ID de Tarea',
  'Task ID:': 'ID de Tarea:',
  'Copy Public Share Link': 'Copiar Enlace Público',
  'Download as PDF': 'Descargar como PDF',
  'PDF download will be implemented soon': 'La descarga de PDF se implementará pronto',

  // Task Detail Page
  'Back to Tasks': 'Volver a Tareas',
  'Task': 'Tarea',
  'Solution': 'Solución',
  'Images': 'Imágenes',
  'Created by': 'Creado por',
  'Copy Share Link': 'Copiar Enlace',
  'Download PDF': 'Descargar PDF',
  'Share link copied to clipboard!': '¡Enlace copiado al portapapeles!',
  'review': 'reseña',
  'reviews': 'reseñas',

  // Subscription Page
  'Manage Your Subscription': 'Gestiona Tu Suscripción',
  'View and manage your subscription plan and credits': 'Ver y gestionar tu plan de suscripción y créditos',
  'Current Plan': 'Plan Actual',
  'Trial Plan': 'Plan de Prueba',
  'Annual Plan': 'Plan Anual',
  'No Active Plan': 'Sin Plan Activo',
  'Status': 'Estado',
  'Active': 'Activo',
  'Expired': 'Expirado',
  'Cancelled': 'Cancelado',
  'Trial Period': 'Período de Prueba',
  'Started': 'Inicio',
  'Expires': 'Expira',
  'Subscription Period': 'Período de Suscripción',
  'Task Generation Credits': 'Créditos de Generación de Tareas',
  'Remaining Credits': 'Créditos Restantes',
  'credits': 'créditos',
  'You have {{count}} task generation credits remaining.': 'Tienes {{count}} créditos de generación de tareas restantes.',
  'Annual Subscription': 'Suscripción Anual',
  'Subscribe to our annual plan for unlimited access to all features.': 'Suscríbete a nuestro plan anual para acceso ilimitado a todas las funciones.',
  'Unlimited task generation': 'Generación ilimitada de tareas',
  'Priority support': 'Soporte prioritario',
  'Early access to new features': 'Acceso anticipado a nuevas funciones',
  'Subscribe Now': 'Suscribirse Ahora',
  'Coming Soon': 'Próximamente',
  'Annual subscription will be available soon. Stay tuned!': '¡La suscripción anual estará disponible pronto. Mantente atento!',

  // Registration Success Message
  'Registration successful, your 3-month free trial subscription just started!': '¡Registro exitoso, tu suscripción de prueba gratuita de 3 meses acaba de comenzar!',
};
